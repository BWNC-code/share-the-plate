...
class TestViews(TestCase):
    ...
    def test_profile_info(self):
        ...
        self.assertEqual(response.context['user'].username, self.username) # Check if the correct user is retrieved
    ...
    def test_user_recipes(self):
        ...
        self.assertEqual(len(response.context['recipes']), 1) # Check if the correct number of recipes is retrieved
        self.assertEqual(response.context['recipes'][0].title, 'Test Recipe') # Check if the correct recipe is retrieved
    ...
    def test_liked_recipes(self):
        ...
        self.assertEqual(len(response.context['liked_recipes']), 1) # Check if the correct number of liked recipes is retrieved
        self.assertEqual(response.context['liked_recipes'][0].title, 'Test Recipe') # Check if the correct liked recipe is retrieved
    ...
    def test_recipe_detail(self):
        ...
        self.assertEqual(response.context['recipe'].title, 'Test Recipe') # Check if the correct recipe is retrieved
    ...
    def test_search(self):
        ...
        self.assertEqual(len(response.context['results']), 1) # Check if the correct number of search results is retrieved
        self.assertEqual(response.context['results'][0].title, 'Test Recipe') # Check if the correct search result is retrieved

    def test_profile_info_invalid_user(self):
        response = self.client.get(reverse('share_the_plate:profile_info', kwargs={'username': 'invalid_user'}))
    ...
    def test_user_recipes(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'share_the_plate/user_recipes.html')
        self.assertEqual(len(response.context['user_recipes']), 1) # Check if the correct number of recipes is retrieved

    def test_liked_recipes_invalid_user(self):
        response = self.client.get(reverse('share_the_plate:liked_recipes', kwargs={'username': 'invalid_user'}))
        self.assertEqual(response.status_code, 404)

    def test_deactivate_account_invalid_user(self):
        response = self.client.get(reverse('share_the_plate:deactivate_confirm', kwargs={'username': 'invalid_user'}))
        self.assertEqual(response.status_code, 404)

    def test_toggle_like_invalid_recipe(self):
        response = self.client.post(reverse('share_the_plate:toggle_like', kwargs={'slug': 'invalid-recipe'}))
        # Assuming you return a 404 status for invalid recipes
        self.assertEqual(response.status_code, 404)

    def test_recipe_detail_invalid_recipe(self):
        response = self.client.get(reverse('share_the_plate:recipe_detail', kwargs={'slug': 'invalid-recipe'}))
        self.assertEqual(response.status_code, 404)

    def test_search_invalid_query(self):
        response = self.client.get(reverse('share_the_plate:search'), {'q': 'Invalid Recipe'})
        self.assertEqual(response.status_code, 200)
